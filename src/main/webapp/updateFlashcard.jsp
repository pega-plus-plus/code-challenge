	...
	<div class="container-fluid">
		<nav class="navbar">
			<a class="navbar-brand" href="index.jsp">
				<h1>
					Flashcards! <small>Update Flashcard</small>
				</h1>
		...
		<form method="POST" action="UpdateFlashcardServlet" id="updateFlashcard">
			<input type="hidden" class="form-control" id="flashcardId" name="flashcardId"
				required="required" value="${fc.flashcardId }" />
			<div class="form-group">
				<label for="category">Category</label> 
				<select required="required" class="form-control" name="categoryId" id="categoryId">
					<option value="0">Select Category</option>
					<c:forEach var="cat" items="${catList}">
						<option value="${cat.categoryId}"
							<c:if test="${fc.categoryId == cat.categoryId}">
								selected
							</c:if>>
